...
private[sql] abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  object AggregateOperator extends Strategy {
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case logical.Aggregate(groupingExpressions, resultExpressions, child)
        ...
        val aggregateExpressionMap = aggregateExpressions.zipWithIndex.map {
          case (agg, index) => agg -> Alias(agg, s"_agg$index")().toAttribute
        }.toMap
        ...
        val postShuffleAggregateAttributes =
          postShuffleAggregateExpressions.map(aggregateExpressionMap)
        val rewrittenResultExpressions = resultExpressions.map { expr =>
          expr.transform {
            case agg: AggregateExpression2 => aggregateExpressionMap(agg).toAttribute
