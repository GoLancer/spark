  ...
  protected override def doExecute(): RDD[InternalRow] = attachTree(this, "execute") {
    child.execute().mapPartitions { iter =>
      new Iterator[InternalRow] {
        ...
        private val buffer: MutableRow = {
          val size =
            if (preShuffle) {
              aggregateAttributes.length
            } else {
              groupingExpressions.length + aggregateAttributes.length
            }
          ...
          while (iter.hasNext && !findNextPartition) {
            ...
            if (comparing == 0) {
              ...
              if (preShuffle) {
                var i = 0
                while (i < aggregateFunctions.length) {
                  aggregateFunctions(i).update(buffer, currentRow)
                  i += 1
                }
              } else {
                var i = 0
                while (i < aggregateFunctions.length) {
                  aggregateFunctions(i).merge(buffer, currentRow)
                  i += 1
                }
              }
            } else {
          ...
          if (hasNext) {
            val outputRow =
              if (preShuffle) {
                ...
                joinedRow(currentGroupingKey, buffer)
              } else {
                var i = 0
                while (i < aggregateFunctions.length) {
